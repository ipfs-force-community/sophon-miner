...
func newSlashFilterAPI(sfCfg *config.SlashFilterConfig) Option {
	return func(settings *Settings) error {
		switch slashfilter.BlockStoreType(sfCfg.Type) {
		case slashfilter.Local:
			Override(new(slashfilter.SlashFilterAPI), slashfilter.NewLocal)
		case slashfilter.MySQL:
			Override(new(slashfilter.SlashFilterAPI), slashfilter.NewMysql(&sfCfg.MySQL))
		default:

		}

		return fmt.Errorf("unsupported block store type [%s]", sfCfg.Type)
	}
}
...
func ConfigMinerOptions(cctx *cli.Context, c interface{}) Option {
	...
	minerOps := Options(
		newSlashFilterAPI(cfg.SlashFilter),
